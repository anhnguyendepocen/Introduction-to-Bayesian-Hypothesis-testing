...
plot.beta <- function(PS = 1, PF = 1, k = 0, n = 0, null = 0.5, CI = NULL, ymax = "auto", main = NULL, map = FALSE, ml=FALSE) {
...
BF <- function(PS, PF, n, k, null){
  prior <- dbeta(null, PS, PF) # data for prior curve
  posterior <- dbeta(null, PS + k, PF + n - k) # data for posterior curve
  posterior/prior
}
   ...
   sidebarLayout(
      sidebarPanel(
        tabsetPanel(tabPanel("Prior", numericInput("alpha", "Prior success", 1, min=0),
                                      ...
                                      sliderInput('null', "Hypothesis", 0.5, min=0, max=1)),
...
server <- function(input, output) {
   output$distPlot <- renderPlot({
      ...
      plot.beta(PS = input$alpha, PF = input$beta, k = sum(data), n = input$n, null=input$null, CI = 0.95, 
                main=paste0("Observing ", sum(data), " Heads in ", input$n, " coin flips"))
   })
   output$bayes_factor <- renderUI({
     generate_data <- repeatable(rbinom, seed=input$seed)
     data <- generate_data(input$n, 1, input$prob)
     bf <- BF(input$alpha, input$beta, input$n, sum(data), input$null)
     div(HTML(paste0("<h4>Bayes Factor</h4><p>Comparing point hypothesis (H<sub>0</sub>) to alternative (H<sub>1</sub>)</p><p>BF<sub>01</sub> = ", 
                     round(bf,3), 
                     "<br/>BF<sub>10</sub> = ",
                     round(1/bf, 3),
                     "</p>")))
